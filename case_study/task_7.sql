...
SELECT
   ...
   sum(IFNULL((IFNULL((hdct.so_luong * dvdk.gia),0)+dv.chi_phi_thue),0)) AS tien_thanh_toan,
...
WHERE ma_khach_hang IN (SELECT hd.ma_khach_hang FROM hop_dong hd WHERE YEAR(hd.ngay_lam_hop_dong)<2021);
